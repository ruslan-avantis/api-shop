    ...
    public function paginator($totalItems)
    {
        ...
        if ($number_pages >= 2) {
            foreach($pages as $key => $pag_item)
            {
                ...
                $next = '';
                if ($pag_item == 'first') {
                    $previous = '<li><a href="'.$pagination_url.'">&larr; '.$real_key.'</a></li>';
                }
                if ($pag_item == 'less') {
                    $previous = '<li><a href="'.$pagination_url.'">'.$real_key.'</a></li>';
                }
                if ($pag_item == 'previous') {
                    $previous = '<li><a href="'.$pagination_url.'">'.$real_key.'</a></li>';
                }
                if ($pag_item == 'current') {
                    $next = '<li class="active"><a href="#">'.$real_key.'</a></li>';
                }
                if ($pag_item == 'next') {
                    $next = '<li><a href="'.$pagination_url.'">'.$real_key.'</li>';
                }
                if ($pag_item == 'more') {
                    $next = '<li><a href="'.$pagination_url.'">'.$real_key.'</a></li>';
                }
                if ($pag_item == 'last') {
                    $previous = '<li><a href="'.$pagination_url.'">'.$real_key.' &rarr;</a></li>';
                }
